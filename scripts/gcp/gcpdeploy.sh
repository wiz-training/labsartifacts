...
gcloud compute instances create $vm_name \
  ...
  --tags=http-server \
  --create-disk=auto-delete=yes,boot=yes,device-name=mygcpvm,image=projects/ubuntu-os-cloud/global/images/ubuntu-1804-bionic-v20240116a,mode=rw,size=10,type=projects/"$project_name"/zones/us-central1-a/diskTypes/pd-balanced \
  --no-shielded-secure-boot --shielded-vtpm --shielded-integrity-monitoring \
  --labels=goog-ec-src=vm_add-gcloud \
  --reservation-affinity=any

gcloud compute --project=$project_name firewall-rules create http \
  --direction=INGRESS --priority=1000 --network=default \
  --action=ALLOW --rules=tcp:80 --source-ranges=0.0.0.0/0

declare -l bucket_name
bucket_name="mybucket-$(date +%s | sha256sum | base64 | head -c 8)"
gsutil mb -p $project_name -c STANDARD -l us-central1 gs://$bucket_name/
